...
const config = {
    A187: 'ExTRUPÍCIO 🦍',
    instagram: 'OFF',
    nomer: 'https://wa.me/qr/APD4JEYHX3AXE1',
    aktif: '24 JAM',
    youtube: 'OFF',
    whatsapp: 'https://chat.whatsapp.com/CjmeGTn1gMtEXABi0IhqCE',
   ...
   switch (prefix) {
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[SEGURA AS PONTAS AI 🐌', MessageType.text)
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   let hasil = `TOMA O LINK AI OTAKU 🦸:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[SEGURA AS PONTAS AI 🐌', MessageType.text)
       ...
       case 'tiktok':
           tiktokdl(value)
               .then(data => {
                    ...
                    let hasil = `✄1�71 ￄ 1/77 DEU BOM 😎 !!! Clique no link abaixo para baixar os resultados! \nClique no link abaixo🗡️\n\nTítulo: ${deskripsi} \n\nDurasi: ${durasi}\n\nNome: ${nama}\n\nUrl: ${url}`;
                    client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'fb':
       ...
       case 'ocr':
           ...
           readTextInImage(media)
               .then(data => {
                   client.sendMessage(id, `*ENTÃO, TOMA UMA FRASE AI OTAKU 🧐 * \n\nResultado:: \n\n${data}`, MessageType.text);
               })
               .catch(err => {
                   console.log(err)
               })
...
case 'Nível gay':
           gay()
               .then(data => {
                   ...
                   let hasil = `*${desc} \n\n *MANO TU É?? OU SOU EU QUE DEIXO 🤔* _${persen}_  `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
